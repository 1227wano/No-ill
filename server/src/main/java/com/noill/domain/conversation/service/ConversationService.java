...
public class ConversationService {
    ...
    private boolean isSessionExpired(Talk talk) {
        ...
        // 메시지가 하나도 없다면 -> 갓 생성된 세션이므로 만료 아님
    ...
    public String getConversationHistory(Talk talk) {
        // 단기 기억: 현재 세션의 '모든' 대화 내용을 가져옵니다.
        List<Message> sessionMessages = messageRepository
                .findAllByTalk_TalkNoOrderByCreatedAtAsc(talk.getTalkNo());

        StringBuilder sb = new StringBuilder();
        for (Message m : sessionMessages) {
            sb.append(m.getMsgType().equals("Q") ? "User: " : "Noill: ");
    ...
    public String getRelatedMemories(Pet pet, String userText) {
        ...
        for (String word : words) {
            ...
            // 2. 키워드로 종료된 세션 제목 검색
            List<Talk> memories = talkRepository.findByPet_PetNoAndStatusAndTalkNameContaining(
                    pet.getPetNo(), "N", word);
        ...
        // 찾은 게 있으면 목록 문자열, 없으면 뒤 문장 return
    ...
    public String getConversationFullContext(Talk talk) {
        List<Message> messages = messageRepository.findAllByTalk_TalkNoOrderByCreatedAtAsc(talk.getTalkNo());
        ...
        for (Message msg : messages) {
            String role = "Q".equals(msg.getMsgType()) ? "User" : "Noill";
