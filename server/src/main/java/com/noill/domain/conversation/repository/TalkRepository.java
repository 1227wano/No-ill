...
public interface TalkRepository extends JpaRepository<Talk, Long> {
    ...
    java.util.List<Talk> findByPet_PetNoAndStatusAndTalkNameContaining(Long petNo, String status, String keyword);

    /**
     * 일정 시간 동안 대화가 없는 활성 세션 조회 (Batch용)
     * 조건: Status = 'Y' AND MAX(Message.createdAt) <= threshold
     */
    @org.springframework.data.jpa.repository.Query("SELECT t FROM Talk t " +
            "JOIN t.messages m " +
