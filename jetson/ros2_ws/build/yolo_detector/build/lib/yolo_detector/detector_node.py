...
from std_msgs.msg import Int32, String, Bool  # Bool 추가
import cv2
import numpy as np
import os
...
class YoloDetectorNode(Node):
    def __init__(self):
        ...
        self.pub_cap_status = self.create_publisher(Bool, 'accident_cap', 10) # 캡처 완료 알림

        # ROS2 Subscriber
        self.sub_accident = self.create_subscription(
            Bool, 
            'check_accident', 
        ...
        # 상태 변수
        self.current_frame = None  # 캡처를 위한 최신 프레임 저장용
        self.save_path = os.path.expanduser('~/Downloads') # 이미지 저장 경로
        ...
        self.get_logger().info('YOLOv11: Event-based Capture Mode Enabled.')

    def accident_callback(self, msg):
        """check_accident 토픽이 True이면 캡처 수행"""
        if msg.data is True and self.current_frame is not None:
            ...
            self.get_logger().info(f'Accident Capture Saved: {file_name}')
    ...
    def inference_loop(self):
        ...
        # 현재 프레임을 클래스 변수에 저장 (이벤트 발생 시 이 이미지가 저장됨)
