...
public:
  SafetyOverrideNode() : Node("safety_override_node") {
    ...
    declare_parameter<double>("slow_dist", 0.50);     // 50cm
    declare_parameter<double>("reverse_dist", 0.20);  // 20cm

    declare_parameter<double>("min_fwd", 0.08);       // 20~50cm 구간 최소 전진속도(정지 방지용)
    declare_parameter<double>("rev_speed", -0.08);    // 후진 속도
    ...
    declare_parameter<double>("sector_front_deg", 10.0);
    declare_parameter<double>("sector_side_min_deg", 10.0);
    declare_parameter<double>("sector_side_max_deg", 60.0);
